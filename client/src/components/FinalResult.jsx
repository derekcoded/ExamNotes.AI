...
function FinalResult({result}) {
  const [qucikRevision,setQucikRevision]= useState(false);
  if(!result || 
        !result.subTopics ||
        !result.questions ||
  ...
  return (
    <div className='mt-6 p-3 space-y-10 bg-white'>
      ...
      <div className='flex gap-3'>
        <button
        onClick={()=>setQucikRevision(!qucikRevision)}
        className={`px-4 py-2 rounded-lg text-sm font-medium transition
          ${qucikRevision 
            ? "bg-green-600 text-white"
            : "bg-green-100 text-green-700 hover:bg-green-200"
          }`}> {qucikRevision ? "Exit Revision Mode" : "Quick Revision (5min)"}
     ...
     { !qucikRevision &&<section >
      ...
      border-green-200 p-6'>
       <h3 className='font-bold text-green-700 mb-3 text-lg'>
        ...
        <ul className='list-disc ml-6 space-y-1 text-gray-800'>{result.revisionPoints.map((p,i)=>(
          <li key={i}>{p}</li>
       ) )}</ul>
</section>}
...
{result.diagram?.data &&
  <section>
    ...
    <p className='mt-3 text-xs text-gray-500 italic'>ℹ️ If you need this diagram for future reference or revision,
      you can save it by taking screenshot.
    </p>
...
{result.charts?.length > 0 &&
  <section>
    ...
    <p className='mt-3 text-xs text-gray-500 italic'>ℹ️ If you need this Chart for future reference or revision,
      you can save it by taking screenshot.
    </p>
</section>}
...
<section>
  ...
  <ul className='list-disc ml-6 text-gray-700'>{result.questions.short.map((q,i)=>(
  <li key={i}>{q}</li>
  ))}
  </ul>
  <p className='font-medium'>Long Questions:</p>
  <ul className='list-disc ml-6 text-gray-700'>{result.questions.long.map((q,i)=>(
  <li key={i}>{q}</li>
  ))}
  </ul>
